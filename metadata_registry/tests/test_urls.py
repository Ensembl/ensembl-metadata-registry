...
class TestBasicUrls(TestCase):
    ...
    def test_datarelease_urls(self):
        ...
        self.assertEqual(response.status_code, 200)
        response = self.client.get(reverse('datarelease_database_list'))
        self.assertEqual(response.status_code, 200)
        response = self.client.get(reverse('datarelease_database_event_list'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_genome_urls(self):
        ...
        self.assertEqual(response.status_code, 200)
        for division in self.divisions:
            response = self.client.get(reverse('genomeinfo_ens_table', kwargs={'division': division}))
            self.assertEqual(response.status_code, 200)
    ...
    def test_datatable_urls(self):
        datatables = ['organism', 'assembly', 'division', 'datarelease']
        for table in datatables:
            ...
            self.assertEqual(response.status_code, 200)
            response = self.client.get(reverse('datatablefetch_serverside_datarelease'))
            self.assertEqual(response.status_code, 200)
