...
from metadata_registry_current.models.organism import Organism, OrganismAlias, OrganismPublication
from metadata_registry_current.api.organism.serializers import OrganismSerializer, OrganismAliasSerializer,\
    OrganismPublicationSerializer
from metadata_registry_current.api.organism.filters import OrganismFilterBackend, \
...
class OrganismDatatableViewCurrent(DataTableListApi):
    serializer_class = OrganismSerializer
    search_parameters = SchemaUtils.get_field_names(app_name='metadata_registry_current', model_name='organism', exclude_pk=False)
    default_order_by = 5
    queryset = Organism.objects.all()
